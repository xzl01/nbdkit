...
=head2 C<nbdkit_peer_security_context>
...
label or NetLabel, see the description of C<SO_PEERSEC> in L<unix(7)>
and L<ip(7)>.

On success this returns the security context as a string.  The caller
must free the string.  On error (which can include missing label or
unsupported protocol), C<nbdkit_error> is called and this call returns
C<NULL>.
